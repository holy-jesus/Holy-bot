...
export const Commands: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    if (currentCommand.id) {
      setCommands(
        commands.map((c) =>
          c.id === currentCommand.id
            ? ({ ...c, ...currentCommand } as Command)
            : c,
        ),
      );
    } else {
  ...
  const BasicEditor = () => (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div>
        <label className="block text-sm font-medium text-slate-400 mb-1">
          {t("commands.basicResponse")}
        </label>
        <textarea
          value={currentCommand.response || ""}
          onChange={(e) =>
            setCurrentCommand({ ...currentCommand, response: e.target.value })
          }
  ...
  const AiEditor = () => (
    <div className="space-y-4 animate-in fade-in duration-300">
      <div className="p-4 bg-purple-500/10 border border-purple-500/20 rounded-lg flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-purple-300/70 mt-1">
            This command will use Gemini AI to generate a unique response every
            time it is triggered, based on your instructions.
          </p>
        </div>
      </div>
      ...
      <div>
        <label className="block text-sm font-medium text-slate-400 mb-1">
          {t("commands.aiPromptLabel")}
        </label>
        <textarea
          value={currentCommand.aiPrompt || ""}
          onChange={(e) =>
            setCurrentCommand({ ...currentCommand, aiPrompt: e.target.value })
          }
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <Button
              ...
              variant="secondary"
              className="flex-1 sm:flex-none border-brand-500/50 text-brand-300 hover:text-white hover:bg-brand-500/20"
        ...
        <div className="bg-slate-950 border border-slate-800 rounded-xl overflow-hidden shadow-2xl animate-in slide-in-from-bottom-5 duration-300">
          <div className="p-6 border-b border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900">
            ...
            <div className="flex gap-1 bg-slate-800 p-1 rounded-lg border border-slate-700 w-full md:w-auto">
              <button
                onClick={() =>
                  setCurrentCommand({ ...currentCommand, mode: "basic" })
                }
              ...
              <button
                onClick={() =>
                  setCurrentCommand({ ...currentCommand, mode: "ai" })
                }
              ...
              <button
                onClick={() =>
                  setCurrentCommand({ ...currentCommand, mode: "advanced" })
                }
