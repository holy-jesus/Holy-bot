...
import { LayoutDashboard, Command, Timer, Shield, Settings, LogOut, Bot } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ activePage, onNavigate, onLogout, isOpen }) => {
  const { t } = useLanguage();

  const menuItems = [
    { id: 'overview', label: t('sidebar.overview'), icon: <LayoutDashboard size={20} /> },
    { id: 'commands', label: t('sidebar.commands'), icon: <Command size={20} /> },
    { id: 'timers', label: t('sidebar.timers'), icon: <Timer size={20} /> },
    { id: 'moderators', label: t('sidebar.moderators'), icon: <Shield size={20} /> },
    ...
    >
      <div className="flex flex-col h-full">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          {menuItems.map((item) => (
            <button
              ...
              `}
            >
              <span className={`mr-3 ${activePage === item.id ? 'text-brand-400' : 'text-slate-500 group-hover:text-slate-300'}`}>
                {item.icon}
              </span>
              {item.label}
            </button>
        ...
        <div className="p-4 border-t border-slate-800">
          <button 
            onClick={onLogout}
            className="w-full flex items-center px-3 py-2.5 text-sm font-medium text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors"
          >
